{lib, ...}@inputs:
let
inherit (lib) mkOption mapAttrsRecursive mapAttrsRecursiveCond concatStringsSep flatten collect isList splitString head last replaceStrings setAttrByPath ;
inherit (lib.types) submodule attrsOf anything;
inherit (import ./lib.nix inputs) recursiveMergeNoOverride mapAttrsFunc ;


  mapAttrsToPathValueList = set: (mapAttrsRecursive (path: value: let
    spl = splitString ":" value;
    service = splitString "." (replaceStrings ["%"] ["."] (head spl));
    iface = last spl;
    ip = concatStringsSep "" path;
  in
  [ {inherit ip service; setForIf = if iface=="" then false else true;} ]) set);
  ...
  flatten_attrs_rec = host_ports:
    recursiveMergeNoOverride (
      map(eth_attrs:
        setAttrByPath (eth_attrs.service) ({
...
flatten_attrs_non_rec = host_ports:
  ...
  map_to_ips = set: mapAttrsRecursiveCond
    (as: !(as ? ip)) #this assumes that you dont use port as a path inside your key!
    (_: x: x.ip)
    set;
